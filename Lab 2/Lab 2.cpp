/*
Lab 2 : Ford Fulkerson


Refrences:
-GeeksForGeeks : Understadning the Ford Fulkerson Algo
*/
#include <stdio.h>
#include <stdlib.h>
#include<conio.h>
...
// Returns 1 if found, otherwise 0.
{
	...
	while (head != tail)
	{
		...
		{
			v = edgeTab[i].head;
			if (color[v] == WHITE && edgeTab[i].capacity - edgeTab[i].flow>=scaling_param)
			{
				enqueue(v);
				pred[v] = u;
				predEdge[v] = i;
...
int max_flow(int source, int sink)
{
	...
	for (i = 0; i<residualEdges; i++)
		edgeTab[i].flow = 0;
	while (scaling_param > 0)
	{
		printf(" \nScaling parameter is %d : \n", scaling_param);
...
void read_input_file()
{
	...
	while (scaling_param * 2 < max_capacity)
...
int main()
{
	...
	if (n <= 20)
	{
		...
		for (i = 0; i<residualEdges; i++)
			if (edgeTab[i].flow>0)
				printf("%d->%d has %d\n", edgeTab[i].tail, edgeTab[i].head, edgeTab[i].flow);
	}
